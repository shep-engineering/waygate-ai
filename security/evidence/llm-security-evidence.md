...
| Sanitization module | `agent_api.security` |
...
|---|---|---|---|---|
| S1 | `agent_api/client.py` → `LLMClient.call(system, user)` | Caller-supplied `system` string | Caller's responsibility | Caller's responsibility |
| S1 | `agent_api/client.py` → `LLMClient.call(system, user)` | Caller-supplied `user` string | `sanitize()` provided | `wrap()` provided |

*Note: agent-api is a library. It provides sanitization tools but cannot enforce caller adoption.
...
| Property | Value |
|---|---|
| Module location | `agent_api/security.py` |
