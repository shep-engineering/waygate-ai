# Documentation Implementation Brief — agent-api
...
agent_api/
  ...
  providers/
    anthropic.py      — Wraps anthropic SDK; maps errors to AgentAPIError hierarchy
...
from agent_api import (
    ...
    AgentAPIError, AuthError, ConfigError, RateLimitError, TransientError,
)
```
...
| `.claude/skills/integrate-agent-api/SKILL.md` | **Create** | Claude Code integration skill |
| `.claude/skills/integrate-agent-api/skill.yaml` | **Create** | USF v1.0 permission manifest |
...
flowchart TD
  ...
  Q1 -->|No| Install[pip install agent-api...]
  Install --> Q2
...
1. Detect whether agent-api is already in deps
...
**`.claude/skills/integrate-agent-api/skill.yaml`** (USF v1.0)
- `risk_tier: L1`
...
- [ ] `.claude/skills/integrate-agent-api/SKILL.md` + `skill.yaml` present
