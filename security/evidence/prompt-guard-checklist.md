...
| Check | Status | Notes |
|---|---|---|
| `sanitize(text, content_type)` function exists as a standalone, importable module | ☑ | `waygate_ai/security.py` |
...
| Check | Status | Notes |
|---|---|---|
| `is_safe(text) -> (bool, list[str])` audit hook exists | ☑ | `waygate_ai/security.py::is_safe()` |
