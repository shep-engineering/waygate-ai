...
### 1. Detect whether agent-api is already in dependencies

Check `pyproject.toml`, `requirements*.txt`, or existing imports.

```bash
python -c "import agent_api; print(agent_api.__all__)"
```
...
```python
from agent_api import sanitize, wrap
...
```python
from agent_api import AgentAPIError, AuthError, ConfigError, LLMClient
...
def test_integration_uses_client(monkeypatch, mocker):
    ...
    mocked = mocker.patch(
        "agent_api.providers.openai.call",
