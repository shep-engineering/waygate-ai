...
This guide shows how to add `limen` to an agent or Python application while
preserving the library's security and provider-abstraction contract.

`limen` is intentionally domain-neutral. It does not include application
...
### 1. Detect whether limen is already in dependencies

Check `pyproject.toml`, `requirements*.txt`, or existing imports.

```bash
python -c "import limen; print(limen.__all__)"
```
...
```python
from limen import sanitize, wrap
...
```python
from limen import LimenError, AuthError, ConfigError, LLMClient
